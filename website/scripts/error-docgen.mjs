...
The following reference is a complete list of the errors you may encounter while using Kumo. For additional assistance, including common pitfalls, please also see our [Troubleshooting Guide](/en/guides/troubleshooting/).
`;
			...
			fs.writeFileSync(
				...
				currentContent.replace(
					...
					[
						...
						'',
						getDeprecatedText(
							'This error is from an older version of Kumo and is no longer in use. If you are unable to upgrade your project to a more recent version, then you can consult [unmaintained snapshots of older documentation](/en/upgrade-astro/#older-docs-unmaintained) for assistance.'
						),
					].join('\n')
				),
				'utf8'
	...
	function getDeprecatedText(deprecateMention) {
		...
		return [
			...
			typeof deprecateMention === 'string'
				? deprecateMention
				: 'This error cannot be emitted by Kumo anymore',
			':::',
		].join('\n');
	}
