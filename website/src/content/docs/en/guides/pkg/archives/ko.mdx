...
sidebar:
  label: Docker Images with Ko
description: 'Docker Images with Ko Releases.'
i18nReady: true
---
...
options as the [build][/en/guides/pkg/build/go/] pipe when possible, so the results will probably be
cached.
...
kos:
  - # ID of this image.
    ...
    #
    # Default "kmdopkg.ko.local" - local docker registry is used.
...
:::tip
    ...
    Note that while kmdo's build section will evaluate environment
