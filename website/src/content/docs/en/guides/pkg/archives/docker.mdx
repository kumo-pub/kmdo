...
If we run `kmdo pkg release`, i.e., a production build, it'll build and
publish `user/repo:1.2.3`, for example.

If we run `kmdo pkg release --snapshot`, it'll build two images instead:
