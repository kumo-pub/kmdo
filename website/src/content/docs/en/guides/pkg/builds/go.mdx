...
builds:
  ...
  - #
    ...
    #
    # Default: '-s -w -X main.version={{.Version}} -X main.commit={{.Commit}} -X main.date={{.Date}} -X main.builtBy=kumose'.
    # Templates: allowed.
    ldflags:
...
:::tip
    ...
    Learn more about [build hooks](/en/guides/pkg/builds/hooks/).
...
The binary name field supports [templating](/en/guides/pkg/general/#templates). The
...
Kmdo also provides a `--parallelism` flag to control how many internal
tasks (e.g., builds, archives, uploads) run concurrently.
If `--parallelism` is not set, Kmdo defaults to the current value of
`GOMAXPROCS`.

### `GOMAXPROCS` for `go build` commands

Each `go build` command launched by kmdo inherits the environment,
...
If you want Kmdo to run up to 10 tasks in parallel, but restrict `go
...
- Kmdo to run up to 10 internal tasks concurrently
...
If you use Go 1.11+ with go modules or vgo, when Kmdo runs it may try to
download the dependencies. Since several builds run in parallel, it is very
likely to fail.

You can solve this by running `go mod tidy` before calling `kmdo pkg` or
by adding a [hook](/en/guides/pkg/builds/hooks/) doing that on your `.kmdopkg.yaml` file:
...
- Modify `ldflags`: by default `main.Date` is set to the time Kmdo is run
...
Since you can have Kmdo build for multiple different `GOAMD64` targets, it
...
Kmdo supports compiling and releasing C shared or static libraries, by
...
applied to the build line (via the `-buildmode` flag), but Kmdo will not
