...
When you run kmdo pkg with `--snapshot`, it will set the `Version` template
...
:::note["Maybe you are looking for something else?"]
    ...
    - If just want to build the binaries, and no packages at all, check the [`kmdo pkg build` command](/en/ref/pkg/pkg_build);
:::
