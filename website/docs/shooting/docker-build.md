...
The way Kmdo works, the correct binary for the platform you're building
...
The `ids` property in the Dockers configuration tells Kmdo which build IDs
to include.
...
The "default" context is a built-in context in "docker buildx", and it is automatically created. This context typically points to the local Docker environment and is used by default for building images. It has to be active for `kmdo` to build images with "buildx".
...
:::warning
    ...
    step, you can have a `Dockerfile` just for Kmdo, for example, a
    `kmdo.dockerfile`.
:::