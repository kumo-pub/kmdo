...
You can run your release locally (e.g. `kmdo pub release --snapshot`) and
check the `./dist/*.json` files to debug.

### 3. You are running Kmdo multiple times against the same tag

This one is easier to fix: make sure you are running Kmdo only on tags,
