...
Kmdo supports publishing artifacts by executing a custom publisher.
...
adding the command to your `.kmdopkg.yaml` file:
...
```bash
custom-publisher -product=kumose -version=1.0.0 kmdo_1.0.0_linux_amd64.zip
```

In `/path/to/dist` with `TOKEN=token`, assuming that Kmdo is executed
