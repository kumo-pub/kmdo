...
Kmdo supports this with the `--snapshot` flag and with the `snapshot`
...
When you run Kmdo with `--snapshot`, it will set the `Version` template
...
Note that the idea behind Kmdo's snapshots is for local builds or to
...
:::info "Maybe you are looking for something else?"
    ...
    - If just want to build the binaries, and no packages at all, check the [`kmdo pub build` command](/docs/cmd/pub/pub_build.mdx);
:::
