...
Kmdo's goal is to automate most of the boring work you'll have while
releasing software, ideally using sensible defaults and making the most common
use cases easy.

Kmdo expects a couple of things:

- a `.kmdopkg.yaml` file with the configuration (see the
  [overview](overview.mdx) for more info)
...
A Kmdo run is split into 4 major steps:
...
[command line docs](/docs/cmd/pub/pub.mdx) for details).
