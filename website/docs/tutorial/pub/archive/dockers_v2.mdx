...
:::warning "dockers_v2"
    ...
    It will replace `dockers` and `docker_manifests` in Kmdo v3 (no ETA),
...
To get around this, when we build with `--snapshot`, Kmdo will not build
...
If we run `kmdo pub release`, i.e., a production build, it'll build and
publish `user/repo:1.2.3`, for example.

If we run `kmdo pub release --snapshot`, it'll build two images instead:
...
name and tags of your images to your `.kmdopkg.yaml` file:
...
```sh
docker buildx create --name=kumose --use
