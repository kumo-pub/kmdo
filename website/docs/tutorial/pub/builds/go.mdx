...
:::warning
    ...
    Learn more about [build hooks](hooks.mdx).
:::
...
The binary name field supports [templating](/docs/tutorial/pub/general/templates.mdx). The
...
```sh
GOMAXPROCS=2 Kmdo release --parallelism=10
```
...
You can solve this by running `go mod tidy` before calling `kmdo` or
by adding a [hook](/docs/tutorial/pub/general/hooks.mdx) doing that on your `.kmdopkg.yaml` file:
