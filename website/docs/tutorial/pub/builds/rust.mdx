...
You can use [global after hooks](/docs/tutorial/pub/general/hooks.mdx) to do it:
...
Kmdo will not install Cargo, Rustup, Zig, or cargo-zigbuild for you.
...
You can also add them to your [global before hooks](/docs/tutorial/pub/general/hooks.mdx), e.g.:
...
[^fail]:
    Kmdo will error if you try to use them. Give it a try with
    `kmdo pub r --snapshot --clean`.
