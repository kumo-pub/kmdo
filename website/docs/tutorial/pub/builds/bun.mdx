...
You can now build TypeScript binaries using `bun build --compile` and Kmdo!
...
Kmdo will not install Bun or any other dependencies on which your
workflow depends. Make sure to install them before running Kmdo.

## Caveats

Kmdo will translate Bun's Os/Arch pair into a GOOS/GOARCH pair, so
