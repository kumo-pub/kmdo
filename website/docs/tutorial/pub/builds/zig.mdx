...
You can now build Zig binaries using `zig build` and Kmdo!
...
Kmdo will not install Zig or any other dependencies on which your
workflow depends. Make sure to install them before running Kmdo.

## Caveats

Kmdo will translate Zig's Os/Arch pair into a GOOS/GOARCH pair, so
